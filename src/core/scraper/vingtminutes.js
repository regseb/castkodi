 ...
 *
 * @param {URLMatch} url               L'URL d'une vidéo de 20 Minutes.
...
const action = async (url, metadata, context) => {
    ...
    for (const iframe of doc.querySelectorAll("iframe[data-src]")) {
        const file = await metaExtract(new URL(iframe.dataset.src, url), {
            ...context,
            depth: true,
        });
        if (undefined !== file) {
...
export const extract = matchURLPattern(action, "https://www.20minutes.fr/*");
