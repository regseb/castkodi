...
describe("Scraper: Kickstarter", function () {
    it("should return URL when it's not a video", async function () {
        const url = new URL("https://www.kickstarter.com/projects/playeress" +
                            "/whos-she-a-guessing-game-about-extraordinary" +
                            "-women");
    ...
    it.skip("should return video URL [media]", async function () {
        ...
        const file = await extract(url, options);
        assert.ok(file?.endsWith("/projects/3266544" +
                                 "/video-870029-hls_playlist.m3u8"),
                  `"${file}"?.endsWith(...)`);
    });
});
